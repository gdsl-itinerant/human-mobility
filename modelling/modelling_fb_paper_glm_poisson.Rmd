...
for (i in 1:18){
  ...
  m<-glm(n_crisis ~ z_d + origin_destination_class + z_origin_pop + z_destination_pop + day + hour + wd + z_score, data = dft, family = poisson("log"), weights = start_pop)
...
for (i in 1:18){
  ...
  m<-readRDS(filename)
  # extracting coefficients
  coefficients_df <- broom.mixed::tidy(m, conf.int = TRUE) %>%
    mutate(
      monthyear=monthyear[i]
    )

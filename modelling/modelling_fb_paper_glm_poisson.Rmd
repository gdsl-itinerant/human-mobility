...
timeline<-data_frame(start=c("2020-03-23","2020-06-01","2020-10-14","2021-01-06","2021-03-29"),end=c("2020-06-01","2020-10-14","2021-01-06","2021-03-29","2021-08-01"),name=c("First lockdown","Partial re-opening","Tier system &\nSecond lockdown","Third lockdown","Re-opening"))
...
## Save monthly pre-processed data
```{r}
for (i in 1:18){
  print(monthyear[i])
  dft<-df[df$monthyear==monthyear[i],]
  saveRDS(dft,paste0("~/itinerant/modelling_input/i_",i,".rds"))
}
```
...
for (i in 1:18){
  #print(monthyear[i])
  dft<-readRDS(paste0("~/itinerant/modelling_input/i_",i,".rds"))
  #dft<-df[df$monthyear==monthyear[i],]
  print(summary(dft$month))
  m<-glm(n_crisis ~ z_d + origin_destination_class + z_origin_pop + z_destination_pop + day + hour + wd + z_score, data = dft, family = quasipoisson(link="log"), weights = start_pop)
  filename<-paste0("~/Projects/ITINERANT/modelling_output/v5/m_",i,".rds")
  saveRDS(m,filename)
  print(i)
}
...
for (i in 17:18){
  ...
  filename<-paste0("~/Projects/ITINERANT/modelling_output/v4/m_",i,".rds")
  saveRDS(m,filename)
  print(i)
  gc()
}
...
coefficients<-NULL
for (i in 1:18){
  filename<-paste0("~/itinerant/modelling_output/v3/m_",i,".rds")
  m<-readRDS(filename)
...
## Extraxct coefficients neg binomial

```{r}
coefficients<-NULL
for (i in 1:18){
  filename<-paste0("~/itinerant/modelling_output/v4/m_",i,".rds")
  m<-readRDS(filename)
  # extracting coefficients
  coefficients_df <- broom.mixed::tidy(m, conf.int = T) #%>%
    mutate(
      monthyear=monthyear[i]
    )
  coefficients<-rbind(coefficients, coefficients_df) 
  remove(m)
  print(i)
  gc()
}
...
g<-ggplot()+
  ...
  geom_line(data = data,
            aes(x=monthyear,y=estimate,group=origin_destination_class,colour=origin_destination_class),
            size = 1) + 
  # geom_ribbon(data = data,
  #             aes(x=monthyear,ymin=(ci_lower), ymax=(ci_upper)),
  #             alpha=0.35,
  #             fill="gray36") +
  scale_color_brewer(palette = "Dark2", labels=c("Day","Hour 08","Hour 16","Weekend","Distance (z score)","Population at Destination\n(z score)","Population at Origin\n(z score)","z score"))+
  scale_fill_manual(values = c("#D091BB","#E7F7D5","#D091BB","#B3589A","#9BBF85"))+
  scale_x_date(date_labels = "%b",
               date_breaks = "1 months", #breaks = c("Mar", "Jun", "Sep", "Dec"),
               limits = c(as.Date("2020-03-01"), as.Date("2021-08-01"))
  ...
  theme_minimal()+
  labs(y="Regression Coefficients",color="Co-variates",fill="Timeline")+
...
g1<-ggplot(coefficients_NOD[!(coefficients_NOD$origin_destination_class %in% c("(Intercept)","z_d")),],aes(x=monthyear,y=estimate,group=origin_destination_class,colour=origin_destination_class,fill=origin_destination_class))+
  geom_line(size = 1.5) + 
  ...
  theme_minimal()#+
  facet_grid(rows=vars(origin_destination_class))

png("~/glm-nod_coefficients_nizd.png",units="in", width=15, height=10, res=300)
g1
dev.off()
...
png("~/glm-coeff.png",units="in", width=13, height=10, res=300)
annotate_figure(g, 

...
```
### Multinomial Logistic Regression (aggregate data)

```{r}

movementsGLA_model<-movementsGLA %>%
  mutate(prob_mov=n_movements/start_pop,
         clusterPair=paste0(start_cluster,"_",end_cluster),
         across(where(is.numeric),scale)) 

#eq <- prob_mov ~ start_cluster:end_cluster + dist #removed to facilitate prediction
eq <- prob_mov ~ clusterPair + dist 
m <- glm(eq,
          data=movementsGLA,
          weights = start_pop, 
...
clusters_interaction <- data.frame(clusterPair = unique(movementsGLA$clusterPair))
clusters_interaction$dist<-mean(movementsGLA$dist)
clusters_interaction$probs<-predict(m, newdata = clusters_interaction, "response")
